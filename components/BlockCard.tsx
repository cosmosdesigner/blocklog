...
export const BlockCard: React.FC<BlockCardProps> = ({ block, onResolve, onDelete }) => {
    const [duration, setDuration] = useState(calculateDuration(block.startDate, block.endDate));
    ...
    const statusColor = block.status === BlockStatus.RESOLVED ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400';
            ...
            <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-4">
                <div className="flex-grow">
                    <div className="flex items-center gap-3 mb-2">
                        <span className={`px-3 py-1 text-xs font-semibold rounded-full ${statusColor}`}>{block.status}</span>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center text-sm text-slate-500 gap-x-4 gap-y-1">
                        ...
                        {block.endDate && <span>Resolved: {formatDate(block.endDate)}</span>}
                    </div>
                </div>
